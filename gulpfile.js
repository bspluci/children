...
const config = {
  ...
  doiuse: {
    browsers: ["ie >= 11"],
    ignore: ["css-resize", "css-appearance", "outline"],
    ignoreFiles: [""],
  },
};
...
const paths = {
  ...
  video: {
    src: basedir + "/video/**/*.+(mp4|avi|mkv|wmv|mov)",
    dest: destdir + "/video",
  },
  index: {
...
/*
  ...
  -------------------------------------------------------------------
*/
const serve = gulp.series(gulp.parallel(browserSync, scss, include, watch));

const build = gulp.series(scss, include, optimization, gulp.parallel(images, fonts, js, css, video, video, index));
